...
void TIM8_UP_TIM13_IRQHandler(void)
{
	...
	if(TIM_GetITStatus(TIM13,TIM_IT_Update)==SET) //Òç³öÖÐ¶Ï
	{
		...
		cm1_out = PID_realize_positiontype(&CM1_PID,CM_Real_Speed[0]) - 230;
		cm2_out = PID_realize_positiontype(&CM2_PID,CM_Real_Speed[1]) - 50;
		cm3_out = PID_realize_positiontype(&CM3_PID,CM_Real_Speed[2]) - 50;
		cm4_out = PID_realize_positiontype(&CM4_PID,CM_Real_Speed[3]) - 230;
